...
object PipStepTest extends UnitSpec {
  private val TestCases: Seq[(PipStep, String)] = Seq(
    ...
    (api.PipStep(args=Seq("some argument"), requirements=Seq("a==1").reqs), {
      """{
        |  "args" : [
        |    "some argument"
        |  ],
    ...
    (api.PipStep(args=Seq("arg 1", "arg 2"), requirements=Seq("a==1", "b==2").reqs), {
      """{
        |  "args" : [
        |    "arg 1",
        |    "arg 2"
...
class PipStepTest extends UnitSpec {
  "PipStep" should "store pip arguments and requirements" in {
    ...
    val step = api.PipStep(args=Seq("arg1", "arg2"), requirements=Seq("req1", "req2", "req2").reqs)
  ...
  it should "inherit from a step with requirements" in {
    val step    = api.PipStep(requirements=Seq("a==5", "b==2").reqs)
    val parent1 = api.PipStep(requirements=Seq("c==3", "d==4").reqs)
    val parent2 = api.PipStep(requirements=Seq("a==1", "d==4").reqs)
  ...
  it should "apply defaults to requirements with defaults" in {
    val step1 = api.PipStep(requirements=Seq("a==1", f"b==${Requirement.DefaultVersion}", "c==3").reqs)
    val step2 = api.PipStep(requirements=Seq(f"a==${Requirement.DefaultVersion}", "b==2").reqs)
